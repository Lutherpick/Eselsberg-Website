...
export default async function NewsGrid({ lang }: { lang: "en" | "de" }) {
    ...
    return (
        <section className="grid grid-cols-1 gap-10 border-t border-slate-200/80 pt-8 dark:border-white/10 lg:grid-cols-3">
                ...
                <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
                    {news.map((n) => (
                        <Link
                            ...
                            className="border-t border-slate-200 pt-4 transition hover:border-primary dark:border-white/10 dark:hover:border-secondary"
                        >
            ...
            <div>
                ...
                <div className="mt-5 space-y-3">
                    {appts.length === 0 ? (
                        <div className="border-t border-dashed border-slate-300 pt-4 text-sm leading-6 text-slate-600 dark:border-white/10 dark:text-slate-300">
                            {lang === "de"
                        ...
                        appts.map((a) => (
                            <div key={`${a.date}-${a.title}`} className="border-t border-slate-200 pt-3 dark:border-white/10">
                                <div className="font-sans text-xs font-semibold uppercase tracking-[0.18em] text-primary/70 dark:text-secondary/80">
                                    {a.date}
                                </div>
